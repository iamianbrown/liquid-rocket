...
class Injector:
    ...
    def sizingcalc(self):
        ...
        A_r_m = self.TCAobj.mdot_r / (self.injector_props['C_d'] * np.sqrt(2 * self.injector_props['rho_r'] * self.injector_props['delta_P_o']))  # m^2
        ...
        A_z_m = self.TCAobj.mdot_z / (self.injector_props['C_d'] * np.sqrt(2 * self.injector_props['rho_z'] * self.injector_props['delta_P']))
        ...
        U_r_c = self.TCAobj.mdot_r / (self.injector_props['rho_r'] * A_r_m)
        U_z_c = self.TCAobj.mdot_z / (self.injector_props['rho_z'] * A_z_m)
    ...
    # -------------------------------------------SETTING UP PLOTS-------------------------------------------------#
        ...
        U_r = self.TCAobj.mdot_r / (self.injector_props['rho_r'] * A_r_m)
        U_z = self.TCAobj.mdot_z / (self.injector_props['rho_z'] * np.pi * ((R_p + a) ** 2 - (R_p) ** 2)) #THIS IS WHERE THE CODE BREAKS BECAUSE YOU'RE ADDING THAT 'a' ARRAY SO THE SIZES NEED TO MATCH WITH 'R_p'
